...
def navigate_to_person(target_name):
    # Load YOLO and face recognizer
    model = YOLO("yolov8l.pt")
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    recognizer.read("Face-Recognition/recognizer/trainingdata.yml")
    face_cascade = cv2.CascadeClassifier('Face-Recognition/haarcascade_frontalface_default.xml')
    ...
    cv2.namedWindow("Navigate to Person")
    cv2.setMouseCallback("Navigate to Person", on_mouse)
            ...
            if conf > 0.5:
                ...
                if model.names[cls] == 'person':
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        id, confidence = recognizer.predict(gray[y:y + h, x:x + w])
                        profile = get_profile(id)
                        if profile is not None and profile[1] == target_name:
                            person_detected = True
                            engine.say(f"Navigating to {target_name}")
                            engine.runAndWait()

                            # Continuous instructions until user reaches target
                            while True:
                                ret, frame = cam.read()
                                if not ret:
                                    continue
                                    ...
                                    if conf > 0.5 and model.names[cls] == 'person':
                                        ...
                                        for (x, y, w, h) in faces:
                                            ...
                                            if profile is not None and profile[1] == target_name:
                                                ...
                                                engine.runAndWait()
                                                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
                                                cv2.putText(frame, target_name, (x, y + h + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                                                break
                                        else:
                                            continue
                                        break
